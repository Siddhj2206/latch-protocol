...
describe("the latch edge (Hono app seam)", () => {
  ...
  test("POST /v1/verify: a tampered step-up is hard-failed with its receipt (Act 3)", async () => {
    ...
    const body = await readJson<{ authorized: boolean; reason: string; receipt: { code: string; message: string; clause: string } }>(res);
  ...
  test("POST /v1/verify: the attack matrix hard-fails at the edge (cap, audience)", async () => {
    ...
    expect(await readJson<{ authorized: boolean; reason: string }>(overCap)).toMatchObject({ authorized: false, reason: "AmountCapExceeded" });
    ...
    const audience = await readJson<{ authorized: boolean; reason: string; receipt: { message: string } }>(wrongMerchant);
    expect(audience).toMatchObject({ authorized: false, reason: "AudienceMismatch" });
    expect(audience.receipt.message).toBe("Merchant Mismatch. Expected: mer_sneakerhead, Got: mer_evil");
  });
});